...
// Movies page
    ...
    reviews.forEach(reviewText => {
        ...
        const fullReviewText = $('<pre>').addClass('review-text').text(reviewText);
        const shortenedReviewContainer = $('<pre>').addClass('review-text-shortened-container');
        ...
        const moreIcon = $('<a>').addClass('more-icon').text('...More');
        ...
        $('.review-quote-container .left-quote-icon').after(reviewTextContainer);
    });
    ...
    const textContainer = $('.review-text-container');
    let textIndex = 0;
    let slideTimeout;
    ...
    function autoSlide() {
        ...
        textIndex++;
        if (textIndex > textContainer.length) {
            textIndex = 1;
        }
        textContainer.eq(textIndex - 1).show();
        textContainer.eq(textIndex - 1).addClass('active');
    ...
    function manualSlide(n) {
        ...
        if (n > textContainer.length) {
            n = 1;
        }
        if (n < 1) {
            n = textContainer.length;
        }
        textContainer.eq(n - 1).show();
        textContainer.eq(textIndex - 1).addClass('active');
    }
