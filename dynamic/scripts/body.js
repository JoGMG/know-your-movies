$(document).ready(() => {
  ...
  if (window.location.pathname.includes("movie_page.html")) {
    ...
    let url1 = `https://api.themoviedb.org/3/movie/${IdQuery}/reviews?language=en-US`;
    let url2 = `https://jogmg.pythonanywhere.com/api/movies/${IdQuery}/reviews`;

    Promise.all([fetch(url1, options), fetch(url2, options1)])
    ...
    // Removes .status-message.active class after 5secs
    ...
    // Implement actions when .send-button is clicked
    $(document).on('click keydown', '.send-button', (event) => {
      if (event.type === 'click' || event.which === 13 || event.which === 32) {
        event.preventDefault();

        const userEmailInput = $('#user-email-input').val().trim();
        const userNameInput = $('#user-name-input').val().trim();
        const userReviewInput = $('#user-review-input').val().trim();

        if (userEmailInput === '' && userReviewInput === '') {
          $('#user-email-input').addClass('isempty');
          $('#user-review-input').addClass('isempty');
          $('.askt').addClass('isempty');
          $('.missing-field-message p').text('Email Address and Review are required.');

          $(document).on('click keydown', '#user-email-input', (event) => {
            if (event.type === 'click') {
              event.preventDefault();

              $('#user-email-input').removeClass('isempty');
              $('.user-email .askt').removeClass('isempty');
            }
          });

          $(document).on('click keydown', '#user-review-input', (event) => {
            if (event.type === 'click') {
              event.preventDefault();

              $('#user-review-input').removeClass('isempty');
              $('.user-review .askt').removeClass('isempty');
            }
          });
        } else if (userEmailInput === '') {
          $('#user-email-input').addClass('isempty');
          $('.user-email .askt').addClass('isempty');
          $('.missing-field-message p').text('Email Address is required.');
        ...
        } else if (userReviewInput === '') {
          $('#user-review-input').addClass('isempty');
          $('.user-review .askt').addClass('isempty');
          $('.missing-field-message p').text('Review is required.');
          ...
          $('.missing-field-message p').text('');

          fetch(`https://api.themoviedb.org/3/movie/${IdQuery}?language=en-US`, options)
            .then(response => response.json())
            .then(async response => {
              base_url = "https://image.tmdb.org/t/p/w500";
              const mPoster = base_url + response.poster_path;
              const mTitle = response.title;
              const mDate = response.release_date;
              // const mGenre = response.genres;
              const mSummary = response.overview;

              if (mTitle !== '' && mDate !== '') {
                try {
                  await postMovieReview(
                    mTitle, mDate, mPoster, mSummary, userEmailInput,
                    userNameInput, userReviewInput, IdQuery)

                  $('.status-message p').text('Review sent successfully');
                  $('.status-message').addClass('green');
                  $('.status-message').addClass('active');
                  startTimeout();
                } catch (error) {
                  $('.status-message p').text('There was an Error, try again');
                  $('.status-message').addClass('red');
                  $('.status-message').addClass('active');
                  startTimeout();
                }
              } else if (mTitle === '' || mTitle === null) {
                $('.status-message p').text('Movie Title is not available, can\'t send a review for this movie.');
                $('.status-message').addClass('red');
                $('.status-message').addClass('active');
                startTimeout();
              } else if (mDate === '' || mDate === null) {
                $('.status-message p').text('Movie Date is not available, can\'t send a review for this movie.');
                $('.status-message').addClass('red');
                $('.status-message').addClass('active');
                startTimeout();
              }
    ...
    // Posts Movie Review
    async function postMovieReview(movieTitle, movieReleaseDate, moviePosterPath, movieOverview, userEmailInput, userNameInput, userReviewInput, movieID) {
      ...
      const options2 = {
        ...
        body: JSON.stringify({
          ...
          'content': userReviewInput
        })
      };

      try {
        await fetch(`https://jogmg.pythonanywhere.com/api/movies/${movieID}/reviews`, options2);
      } catch (error) {
        throw error;
      }
    };
